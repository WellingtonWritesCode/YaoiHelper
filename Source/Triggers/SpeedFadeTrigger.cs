...
// a bunch of this is from crystalline
// TODO: add checkboxes for only horizontal or only vertical
...
public sealed class SpeedFadeTrigger : Trigger {
	...
	public static float On_TriggerGetPositionLerp_ApplySpeedFade(On.Celeste.Trigger.orig_GetPositionLerp orig, Trigger self, Player player, PositionModes mode) {
		...
		foreach (SpeedFadeTrigger speedFadeTrigger in level.Tracker.GetEntities<SpeedFadeTrigger>().Cast<SpeedFadeTrigger>()) {
			if (speedFadeTrigger.Trigger == self) {
				return float.Clamp((player.Speed.Length() - speedFadeTrigger.LowerBound) / (speedFadeTrigger.UpperBound - speedFadeTrigger.LowerBound), 0f, 1f); 
			}
		}	
